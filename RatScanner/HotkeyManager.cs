...
using static ShuShuscanner.RatConfig;
using OverlayC = ShuShuscanner.RatConfig.Overlay;
...
internal class HotkeyManager {
	...
	private static void Wrap<T>(Func<T> func) {
		try {
			...
			ShuShuscannerMain.Instance.SetScanStatusKey("ScanStatusError", true, e.Message);
	...
	private void OnNameScanHotkey(object? sender, KeyUpEventArgs e) {
		Wrap(() => {
			ShuShuscannerMain.Instance.NameScan(UserActivityHelper.GetMousePosition());
			if (_last_mouse_click + 500 < DateTimeOffset.Now.ToUnixTimeMilliseconds() && NameScan.EnableAuto) {
				...
				ShuShuscannerMain.Instance.NameScanScreen();
	...
	private void OnIconScanHotkey(object? sender, KeyUpEventArgs e) {
		Wrap(() => ShuShuscannerMain.Instance.IconScan(UserActivityHelper.GetMousePosition()));
	}
